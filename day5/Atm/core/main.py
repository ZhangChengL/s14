...
def user_info(acc_date):
    ...
    meg='''\033[1;34m
     Information of %s:
          ...
          Credit:%s
          Enroll_date:%s
          Expire_date:%s\033[0m
    '''%(acc_date_now['id'],acc_date_now['id'],acc_date_now['money'],acc_date_now['credit'],acc_date_now['enroll_date'],acc_date_now['expire_date'])
    print(meg)
...
def repay(acc_date):
    ...
    if len(repay_money) > 0 and repay_money.isdigit():
        ...
        if new_money:
            print('\033[1;31m余额为：%s\033[0m'%new_money['money'])
    else:
    ...
    if acc_date_tran:
        ...
        if len(tran_money) > 0 and tran_money.isdigit():
            ...
            if new_money:
                ...
                trans_logger.info(
                    "user:%s   action:%s    money:%s   interest:%s" % (transfer_id,'Receivables' , tran_money, '0'))
        else:
            print('\033[1;31m输入错误！\033[0m')
    else:
        print('\033[1;31m未查询到转款ID，请重新确认！\033[0m')
...
def readjust_the_quota():
    ...
    if acc_update:
        ...
        print('\033[1;31m用户不存在！请确认后输入！\033[0m')
